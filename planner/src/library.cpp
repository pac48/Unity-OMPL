...
public:
  ...
  smoothPath(std::vector<State> &pathVec, RRTSearchInput *input) {
    ...
    for (auto it = 0; it < 1; it++) {
      ...
      int numberBasis = input->numBasisPerMeter * range.second + 3; // add some extra to make problem feasible
      ...
      M.block(numPoints, 0, numberBasis, numberBasis) = input->lambda * Eigen::MatrixXd::Identity(numberBasis, numberBasis);
